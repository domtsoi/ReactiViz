	...
	//**************************************************************************************************************************************************
		void init_tunnel(const std::string& resourceDirectory)
			...
			for (int zz = 0; zz < TUNNELSIZE * 10; zz++)
				...
				for (int xx = 0; xx <= TUNNELSIZE; xx++)
					...
					if (xx == TUNNELSIZE || xx == 0) { xarg = 0.0; yarg = 3.1415926 * 0.5; }
					float randvec = frand() * 0.1 + 0.9;
					
					//randvec = 1;
					randvec *= 20.0;
					if (xx == 0) rand_firstvec = randvec;
					else if (xx == TUNNELSIZE) randvec = rand_firstvec;
		...
		void initGeom(const std::string & resourceDirectory)
			...
			for (int ii = 0; ii < SPHERECOUNT; ii++)
				{
				vec3 pos = vec3(
					frand() * SPHEREDISTANCEZ  - SPHEREDISTANCEZ / 2,
					frand() * 5 - 2.5,
...
//	
			...
			//*****************************************************************************************
				...
				void render_tunnel(int width, int height, mat4 P, mat4 V, bool pilotrender, double frametime)
					...
					for (int ii = 0; ii < roundlaser.size(); ii++) { roundlaserpos[ii] = roundlaser[ii].pos; roundlasercol[ii] = roundlaser[ii].col;}
